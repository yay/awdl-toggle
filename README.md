...
If the native control changes appearance but the window and CLI still show the old setting, the widget extension may have failed to launch. After an extension filename change, macOS can retain its previous executable path in the widget host. Restarting that host clears the stale entry:

```sh
pkill -u "$(id -u)" -x chronod
```

macOS automatically relaunches the host; other widgets may briefly reload too. Reopen Control Center and verify a toggle against the window or CLI status.
